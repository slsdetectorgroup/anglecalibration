...
TEST_CASE("test Epics mythenfilereader", "[mythenfilereader][.files]") {

    auto fpath = test_data_path() / "AngleCalibration_Test_Data";

    REQUIRE(std::filesystem::exists(fpath));

    SECTION("read I0 values from hdf5") {
        EpicsMythenFileReader file_reader;

        auto frame = file_reader.read_frame(
            fpath / "ang1up_22keV_LaB60p3mm_48M_a_0320.h5");
